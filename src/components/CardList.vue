<template>
  <section :style="`background: ${props.options.color}`" @drop="onDrop($event, props.options.id)" @dragover.prevent
    @dragenter.prevent>

    <div class="sort-panel" :style="`background: ${props.options.color}`">
      <v-btn class="sort-panel_button" @click="sortCards('asc')" color="blue">По возрастанию рейтинга</v-btn>
      <v-btn class="sort-panel_button" @click="sortCards('desc')" color="blue">По убыванию рейтинга</v-btn>
      <v-btn class="sort-panel_button" @click="resetSort" color="blue">Сбросить сортировку</v-btn>
    </div>
    <div class="title">
      <h2>
        {{ props.options.title }}
